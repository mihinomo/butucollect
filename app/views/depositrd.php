...
$loan = Loan::getRD($lid);
$customer = Loan::rdCustomer($loan['cid']);
...
if(isset($_GET['make_deposit'])){
    if(isset($_COOKIE['login'])){
		...
		$type="rd";
		$rmks=$_POST['rmks'];
		if(Loan::checkGenuine($lid,$dated)=='tru'){
			Loan::insertDeposit($lid,$amount,$dated,$type,$agent,$rmks);
			echo "<script>alert('Invoice : #BTR-".Loan::fetch_invoice($lid)."'); window.location.replace('/index/');</script>";
		}else{
